...
public interface IActionOrchestrator {
    Task<IReadOnlyList<MacroResult>> RunAsync(MacroContextBase ctx, CancellationToken ct = default);
}
public sealed class ActionOrchestrator : IActionOrchestrator {
    ...
    public async Task<IReadOnlyList<MacroResult>> RunAsync(MacroContextBase ctx, CancellationToken ct = default) {
